...
#   hel %>% 
#   dplyr::select(where(~ !all(. == 0))) %>% 
#   hilldiv(., q = 1 , dis = dist) %>%
#   t() %>%
#   as.data.frame() %>%
#   dplyr::rename(functional_gifts = 1) %>%
#   rownames_to_column(var = "animal_code") %>%
#   mutate(functional_gifts = if_else(is.nan(functional_gifts), 1, functional_gifts))
...
alpha_div <-
  ...
  # full_join(functional_gifts, by = 'animal_code') %>%
...
#   scale_color_manual(values = c("#E69F00", "#CC6677", "#56B4E9")) +
#   theme_minimal() + ylab("Effective number of strains") +
#   ggtitle("Functional diversity GIFTs") +
#   theme(panel.grid.major.x = element_blank(),
#         panel.grid.minor.x = element_blank(),
#         legend.position = "none")
...
<!-- ```{r} -->
<!-- m_div_f_gifts <- -->
<!--   lme(functional_gifts ~ seq_depth + sex + breed + treatment + trial * age, -->
<!--       random = ~ 1|pen, -->
<!--       data = alpha_div) -->

<!-- summary(m_div_f_gifts) -->
<!-- ``` -->
