...
# Neutral
beta_q1n <- 
  hel %>% # ¿? or shall we use mag_weighted?
...
#   filter(rowSums(. != 0, na.rm = TRUE) > 0) %>%
#   dplyr::select_if(~!all(. == 0)) %>%
#   hillpair(., q = 1, dist = dist, metric = "S", out = "pair")
```
...
```{r plot_beta_diff, comment="", message = FALSE, warning = FALSE, fig.height = 4, fig.width = 5, fig.fullwidth = TRUE}
# Neutral
p_n <- 
  betadiv_n_dis %>%
  filter(!diff == '7_35') %>%
...
p_f_gifts <- 
  ...
  theme_minimal() +
  theme(legend.position = 'none') +
  ggtitle('Functional GIFTs beta div')

grid.arrange(p_n, p_p, p_f_kegg, p_f_gifts, ncol = 4)
```
...
```{r permanova_betadiv, comment = "", message = FALSE, warning = FALSE}
...
adonis2(b1f_dis_table_gifts ~ trial + sex + breed + treatment * age,
        permutations = perm,
        data = chicken_metadata)
```
...
```{r distance_rda, echo = FALSE, fig.height = 4, fig.width = 7, fig.fullwidth = TRUE}
set.seed(4)
