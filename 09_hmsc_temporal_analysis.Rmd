...
```{r load_model, message = FALSE, warning = FALSE, echo = FALSE}
# Directory
localDir = "."
...
```{r cross_val, message = FALSE, warning = FALSE, echo = FALSE}
set.seed(1)
...
trends <- 
  ...
  as.data.frame() %>%
  dplyr::rename(mag_id = '.') %>%
  left_join(rbind(increasers_df, decreasers_df), by = 'genome') %>%
