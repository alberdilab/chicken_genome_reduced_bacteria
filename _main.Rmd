...
```{r load_mg_data_beta_div, echo = FALSE}
load("data/data_mg.Rdata")
```
...
# Compare individuals from same trial and pen between days 7-21 and 21-35 (Supplementary Fig. S3)

```{r compare_ind, comment = "", echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE}
metadata_2 <-
  chicken_metadata %>%
...
grid.arrange(p_n, p_p, p_f_kegg, ncol = 4)
```


## 6.3 - Effect of design in microbiome composition

### Permanova values for different metrics of beta diversity (Supplementary Table S3)
...
```{r composition, comment = "", echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE}
perm <- how(nperm = 999)
setBlocks(perm) <- with(chicken_metadata, pen)
...
### Distance-based RDA (Fig. 1f)

```{r distance_rda, echo = FALSE, fig.height = 4, fig.width = 7, fig.fullwidth = TRUE, eval = FALSE}
set.seed(4)
...
```{r create_directories_hmsc_temp, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
# Create directories
...
```{r load_mg_data_hmsc_temp, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
# Load data
load("data/data_mg.Rdata")
...
```{r hmsc_tables_temp, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
...
```{r hmsc_formumla, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
# Define XFormula
...
```{r load_model_temp, message = FALSE, warning = FALSE, echo = FALSE}
# Directory
localDir = "."
ModelDir = file.path(localDir, "temporal_hmsc/models")
...
if(!file.exists("temporal_hmsc/models/temporal_model_thin_10_samples_250_chains_4.Rdata")){
  download.file(url = 'https://sid.erda.dk/share_redirect/Bd8UfDO2D6/temporal_model_thin_10_samples_250_chains_4.Rdata',
...
```{r cross_val_temp, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
# Create partitions
set.seed(12)
...
```{r cross_val_first_part, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
set.seed(1)
...
```{r cross_val_second_part, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
set.seed(1)
...
```{r func_structure, message = FALSE, warning = FALSE, echo = FALSE}
...
```{r mag_trends, message = FALSE, warning = FALSE, echo = FALSE}
# Increasing MAGs
...
## 9.5 - Correlate bacterial MCI and response to time (Fig. 2f)
...
```{r mci_response, message = FALSE, warning = FALSE, echo = FALSE}
mci_df <-
  gifts_elements %>%
  as.data.frame() %>% 
...
## 9.6 - Variance partitioning (Supplementary Table S4)

```{r variance_partitioning, echo = FALSE}
VP <- computeVariancePartitioning(m)
plotVariancePartitioning(hM = m, VP = VP)
```


## 9.7 - Phylogenetic correlogram (Fig. 1e)

### Calcultate phylogenetic distance 

```{r load_data_community, echo = FALSE, eval = FALSE}
...
```{r plot_data_community, echo = FALSE, eval = FALSE}
...
```{r phylo_signal, echo = FALSE, eval = FALSE}
mpost <- convertToCodaObject(m)
...
# Chapter 10 - Bacterial community composition

## 10.1 - Load MG data

```{r load_data_mag_comp, message = FALSE, warning = FALSE, echo = FALSE}
load("data/data_mg.Rdata")
```
...
### Plot GIFTs of all bacteria by element (not included in Supplementary)

```{r density_curves, message = FALSE, warning = FALSE}
gifts_elements %>%
  as.data.frame() %>% 
  rownames_to_column(var = 'genome') %>% 
...
```{r community_gifts, message = FALSE, warning = FALSE}
...
```{r save_gifts, message = FALSE, warning = FALSE}
save(elements_com,
     funcs_com,
     domains_com,
...
```{r create_directories_hmsc_bw, echo = FALSE, eval = FALSE}
# Create directories
dir.create("hmsc_bw")
...
```{r load_mg_data_hmsc_bw, echo = FALSE, eval = FALSE}
# Load data
load("data/data_mg.Rdata")
...
```{r hmsc_tables_bw, echo = FALSE, eval = FALSE}
# Genome count table (quantitative community data)
...
```{r define_model_bw, echo = FALSE, eval = FALSE}
# Define TrFormula
TrFormula <- ~MCI
...
```{r hmsc_model_bw, echo = FALSE, eval = FALSE}
m <- Hmsc(Y = YData,
                XData = XData,
                XFormula = XFormula,
                studyDesign = StudyDesign,
...
```{r mcmc_features_bw, echo = FALSE, eval = FALSE}
...
```{r fit_model_bw, echo = FALSE, eval = FALSE}
set.seed(1)
...
## 15.2 - Expression profiles of positively and negatively associated bacteria (Fig. 3c)
...
```{r filter_reduced_mags, echo = FALSE}
positive_mags <- 
  trends_bw %>%
  filter(trend_09 == "positive") %>%
  select(genome) %>%
...
```{r taxonomy_red_mags, warning = FALSE, comments = "", message = FALSE}
reduced_mags1 <-
  mag_subset_elements_2 %>%
...
```{r positives_red_mags, warning = FALSE, comments = "", message = FALSE}
positives <-
  reduced_mags_df %>%
...
```{r negatives_red_mags, warning = FALSE, comments = "", message = FALSE}
negatives <-
  reduced_mags_df %>%
...
## 15.4 - Genomic characteristics of genome reduced MAGs (Fig. 4a)

```{r calculating_bootstraps, warning = FALSE, comments = "", message = FALSE}
gifts <-
  gifts_elements %>%
  as.data.frame() %>% 
  select(where(~ sum(.) > 0))
...
```{r plot_avg_mci, warning = FALSE, comments = "", message = FALSE, eval = FALSE, fig.height = 7, fig.width = 6, fig.fullwidth = TRUE}
mci_df %>%
  ggplot()+
...
```{r calculating_bootstraps_length, echo = FALSE}
...
```{r plot_length, echo = FALSE, fig.height = 7, fig.width = 6, fig.fullwidth = TRUE}
length_df %>%
  ggplot() +
...
### Expression profiles of genome reduced MAGs (Fig. 4c)

```{r pcoa_data, warning = FALSE, comments = "", message = FALSE, eval = FALSE}
comp_reduced_mags <- 
  reduced_mags_df %>% 
  left_join(genome_taxonomy, by = 'genome') %>%
  filter(family == "UBA1242" | family == "UBA660") %>%
...
```{r pcoa, warning = FALSE, comments = "", message = FALSE, eval = FALSE, fig.height = 12, fig.width = 12, fig.fullwidth = TRUE}
ggplot() +
