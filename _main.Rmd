...
The resulting plot corresponds to Fig. 1a in the manuscript.
...
```{r density_histogram, comment = "", message = FALSE, warning = FALSE, fig.height = 4, fig.width = 6, fig.fullwidth = TRUE}
ggplot(count_taxa, aes(x = total_ind, colour = phylum, fill = phylum)) +
  ...
  scale_x_log10() +
  theme_bw() +
  xlab("Log transformed relative abundance") +
  ylab("Density") +
...
The resulting plots correspond to Figures 1b and 1c in the manuscript. 

```{r tsne_dotplot, comment = "", message = FALSE, warning = FALSE, fig.height = 10, fig.width = 6, fig.fullwidth = TRUE}
tsne_taxonomy <- 
  tsne_df %>%
...
grid.arrange(tsne_taxonomy, tsne_mci, nrow = 2)
```

### Interactive plot  

The plot is not included in the paper but enables to explore each MAG in detail.
...
The resulting plot corresponds to Supplementary Figure S1 in the manuscript.
...
The resulting plots correspond to Supplementary Figure S2 and S3 in the manuscript.
...
The resulting plot corresponds to Figure 2a in the manuscript.
...
The resulting plot corresponds to Supplementary Figure S3 in the manuscript.
...
The resulting plot corresponds to Figure 2c in the manuscript.
...
The resulting plot corresponds to Figure 2d in the manuscript.
...
The plot is not in the Supplementary, but for a more detailed exploration
...
Figure not added to Supplementary but could be included.
...
```{r func_structure, comment = "", message = FALSE, warning = FALSE, echo = FALSE}
...
```{r mag_trends, comment = "", message = FALSE, warning = FALSE, echo = FALSE}
# Increasing MAGs
...
```{r load_data_mag, comment = "", message = FALSE, warning = FALSE, echo = FALSE}
load("data/data_mg.Rdata")
trends <- read_tsv("data/hmsc_mag_trend.tsv")
```

```{r mci_response, comment = "", message = FALSE, warning = FALSE, echo = FALSE}
mci_df <-
  gifts_elements %>%
  as.data.frame() %>% 
...
The resulting results correspond to Supplementary Table S4 in the manuscript.

```{r variance_partitioning, comment = "", message = FALSE, warning = FALSE, echo = FALSE}
...
The resulting plot corresponds to Figure 1e in the manuscript.
...
```{r phylo_signal, comment = "", echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE}
mpost <- convertToCodaObject(m)
...
The resulting plot corresponds to Figure 3a in the manuscript.

```{r calculate_lms_neutral, comment = "", message = FALSE, warning = FALSE}
div_all_day_35 <-
  alpha_div %>%
  filter(sampling_time == '35') %>%
  filter(trial != 'CC') %>% 
...
```{r calculate_lms_p, comment = "", message = FALSE, warning = FALSE}
...
```{r calculate_lms_f, comment = "", message = FALSE, warning = FALSE}
...
```{r mci_bw, comment = "", message = FALSE, warning = FALSE}
domains_com_2 <-
  funcs_com %>%
...
```{r selected_plots_bio, fig.height = 4, fig.width = 10, fig.fullwidth = TRUE}
grid.arrange(p_Mci_mg, p_B04_mg, p_B08_mg, ncol = 3)
```

### All

```{r combined_plots_bio, fig.height = 10, fig.width = 10, fig.fullwidth = TRUE}
...
```{r combined_plots_deg, fig.height = 10, fig.width = 10, fig.fullwidth = TRUE}
...
```{r mci_element_plot, comment = "", message = FALSE, warning = FALSE}
gifts_bw_pos <-
  gifts %>%
